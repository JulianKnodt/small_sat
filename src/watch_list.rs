...
impl WatchList {
  ...
  pub fn clause_decay(&mut self) {
    self.activities.retain(|act| {
      act
        .upgrade()
        .map(|act| {
          let mut data = act.load(Ordering::Relaxed);
          while let Err(v) = act.compare_exchange(
            data,
            (data as f64 * 0.9) as u64,
            Ordering::Relaxed,
            Ordering::Relaxed,
          ) {
            data = v;
